...
def ReadConf():
    data = json.loads(
        requests.get(
            url=f'https://raw.githubusercontent.com/{github_name_}/FGODailyBonusLog/NA/cfg.json', verify=False
        ).text
    )
...
def UploadFileToRepo(filename, content, commit='updated'):
    url = f'https://api.github.com/repos/{github_name_}/FGODailyBonusLog/contents/' + filename + f'?ref=NA'
